...
- 🟢 **Cảnh báo lệch tỷ trọng (Rebalancing Alerts)**: Đã có DB Notification, luồng tự động quét và sinh ra Notification khi tài sản thị trường làm lệch tỷ trọng so với IPS.
- 🟢 **Automatic Asset Price Sync**: Thiết kế `PriceProvider` interface, `ProviderRegistry`. Triển khai DNSE API (Stocks), Fmarket (Funds), Mock Gold provider, `PriceSyncService` và endpoint `POST /api/v1/price-sync`.
